...
class AddNewFundAdvanced extends Component {
  ...
  handleClickEvent(event) {
    if (
      this.state.displayPolicyPopup &&
      this.policyPopupRef &&
      !this.policyPopupRef.current.contains(event.target)
    ) {
  ...
  toggleProtocol(address, action) {
    if (action == "whitelist") {
      if (this.state.whiteListingAdapters.includes(address)) {
        this.setState({
          whiteListingAdapters: this.arrayRemove(
            this.state.whiteListingAdapters,
            address
          ),
        });
      } else {
      ...
      if (this.state.blackListingAdapters.includes(address)) {
        this.setState({
          blackListingAdapters: this.arrayRemove(
            this.state.blackListingAdapters,
            address
          ),
        });
      } else {
  ...
  async toggleAsset(address, action) {
    if (action == "whitelist") {
      if (this.state.whiteListingAssets.includes(address)) {
        await this.setState({
          whiteListingAssets: this.arrayRemove(
            this.state.whiteListingAssets,
            address
          ),
        });
      } else {
      ...
      if (this.state.blackListingAssets.includes(address)) {
        await this.setState({
          blackListingAssets: this.arrayRemove(
            this.state.blackListingAssets,
            address
          ),
        });
      } else {
  ...
  checkWarning() {
    ...
    if (
      this.state.selectedPolicy == "Asset Whitelist" &&
      this.state.whiteListingAssets.length > 0 &&
      !this.state.whiteListingAssets.includes(this.state.denominationAddress)
    ) {
      wtext =
        "Warning: If you are going to use an Asset Whitelist Adaptor, you need to whitelist your denomination asset.";
    } else if (
      this.state.selectedPolicy == "Asset Blacklist" &&
      this.state.blackListingAssets.includes(this.state.denominationAddress)
    ) {
      wtext =
        "Warning: If you are going to use an Asset Blacklist Adaptor, you cannot blacklist your denomination asset.";
    } else {
      wtext = "";
    }
    this.setState({
  ...
  renderPoliciesListWhitelisting() {
    const preState = this;
    return this.state.protocols.map((item) => (
      <div
        className={
          this.state.whiteListingAdapters.includes(item.address)
            ? "w-add-new-fund-denomination-asset-row-selected"
            : "w-add-new-fund-denomination-asset-row"
        }
        key={item.address}
        onClick={() => {
  ...
  renderPoliciesListBlacklisting() {
    ...
    return this.state.protocols.map((item) => (
      <div
        className={
          this.state.blackListingAdapters.includes(item.address)
            ? "w-add-new-fund-denomination-asset-row-selected"
            : "w-add-new-fund-denomination-asset-row"
        }
        key={item.address}
  ...
  renderAssetsListWhitelisting() {
    const preState = this;
    return this.state.assetList.map((item) => (
      <div
        className={
          this.state.whiteListingAssets.includes(item.id)
            ? "w-add-new-fund-denomination-asset-row-selected"
            : "w-add-new-fund-denomination-asset-row"
        }
        key={item.id}
  ...
  renderAssetBlacklisting() {
    ...
    return this.state.assetList.map((item) => (
      <div
        className={
          this.state.blackListingAssets.includes(item.id)
            ? "w-add-new-fund-denomination-asset-row-selected"
            : "w-add-new-fund-denomination-asset-row"
        }
        key={item.id}
    ...
    return (
      <>
        <div className="w-add-new-fund-policy-popup-wrapper">
          <div
            ref={this.policyPopupRef}
            className="w-add-new-fund-policy-popup-box"
          >
        ...
        <div className="w-add-new-fund-step-card">
          ...
          <div className="w-add-new-fund-step-input-section">
            ...
            <div className="w-add-new-fund-advanced-info">
              ...
              <b>
                {" "}
                It is really important to have this number greater than 0, so
                flash loan/arbitrage attacks are not possible on your fund.
              </b>
            </div>
            <div
